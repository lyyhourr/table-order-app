...
export default function Page() {
  ...
  useEffect(() => {
    async function fetchData() {
      const { data } = await api.get<TUserMenu>("/user-menus");
      const { data: categories } = await api.get<TUserMenu>(
        "/user-menus/categories"
      );
      setMenuItems(data as unknown as MenuItem[]);
      setCategoryList(categories as unknown as Category[]);
    }
    fetchData();
  }, []);
